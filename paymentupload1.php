...
// This page is built to accomodate pupils given discount
// here your can now enter the specific amount payable by the pupil at the time the 
// time the pupil is ready to make the first payment

if (isset($_POST["submit"])){
    $username = $_SESSION['username'];

    
    // $pupilname = mysqli_real_escape_string($link, $_REQUEST['pupilname']);
    ...
    // $totalPayment = $amtpaid;
    // $balance = $amountpayable -  $totalPayment;

    
         $sql = "UPDATE payment SET  `admission_no` = '$pupiladminno', `amount_payable`= '$amountpayable', `purpose_of_payment`= '$paypurpose', `Balance`=`amount_payable` - `total_payment`, `created_on` = NOW() WHERE `admission_no` = '$pupiladminno' AND `purpose_of_payment`= '$paypurpose'";
   ...
   <!-- <input type="submit" name="submit" onsubmit="sub()" class="btn btn-card" value="Create"> -->
    ...
    <script type="text/javascript">
        $(document).ready(function () {
          ...
          // function modalInfo(inInfo,outInfo){
          //   var input = document.getElementById(inInfo).value;
          //   var output = document.getElementById(outInfo);
